	...
	if (isset($_POST['new-bug-submit']))
	{
		if ($_POST['new-bug-title'] != '' && $_POST['new-bug-desc'] != '')
		{
			if ($genoptions['showcaptcha'] == true)
			{
				...
				{
					$valid = 0;
					$validmessage = __('C&oacute;digo de confirmação n&atilde;o preenchido', 'bug-library') . ".";
				}
				else
				{
					if ( isset($_COOKIE['Captcha']) )
					{
						...
						{
							$valid = 0;
							$validmessage = __('C&oacute;digo incorreto.', 'bug-library') . ".";
						}
						elseif( (time() - 5*60) > $Time)
						{
							$valid = 0;
							$validmessage = __('C&oacute;digo v&aacute;lido por 5 minutos.', 'bug-library') . ".";
						}
						else
						{
							$valid = 1;					
					...
					{
						$valid = 0;
						$validmessage = __('Cookie do c&oacute;digo faltando. Permita cookies nas configura&ccedil;&otilde;es de seu navegador', 'bug-library') . ".";
					}
				}
			}
			else
		...
		{
			$valid = 0;
			$validmessage = __("Faltando campos obrigat&oacute;rios. Por favor complete-os.", 'bug-library');
		}
	}		
...
Descri&ccedil;&atilde;o <span id='required'>*</span><br />
